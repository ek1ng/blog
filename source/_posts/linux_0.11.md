...
我们先来介绍一下什么是分页机制，之前我们说过在保护模式下，我们在代码中的内存地址要经过分段机制的转换才能变成物理地址。

![图 18](https://s2.loli.net/2022/06/09/WbjqJmkY2ZBFhXL.png)  

这是在没有开启分页机制的时候，我们只需要这一步转换，但是开启分页机制后寻址方式会有一些变化。

![图 19](https://s2.loli.net/2022/06/09/XW3vSpndtBuVRJQ.png)
...
![图 20](https://s2.loli.net/2022/06/09/YFs9ympHDuGC78h.png)  
...
![图 21](https://s2.loli.net/2022/06/09/suNywOzajI91UQ6.png)  

之后我们开启分页机制的开关，就像开启保护模式一样，修改cr0寄存器中的对应位即可。

![图 22](https://s2.loli.net/2022/06/10/8iD1pHzIeqCkSQL.png)  
...
`after_page_tables`这个标签中，前面的`pushl`是跳转到main函数，不过在细讲main函数之前（我也不知道为什么要放在后面开启分页机制？？），我们先看看`jmp setup_paging`，这是开启分页机制。
...
![图 23](https://s2.loli.net/2022/06/10/XIxtAhKBm9pNUYL.png)  
...
这相当于告诉cr3寄存器，0地址出就是页目录表，那我们就可以通过页目录表来找到所有页表然后来访问所有对应的地址啦。

至此，内存布局如下

![图 24](https://s2.loli.net/2022/06/10/SjOIR5TZqdx1omz.png)  
